...
- ⚠⚠ **D: est en exFAT → `npm install` IMPOSSIBLE sur D:** (les symlinks des workspaces npm échouent en EISDIR, non convertible en NTFS). D'où **DEUX clones** :
  ...
  - 🔗 **Remote `dlocal` entre les deux clones** : le clone C: possède un remote **`dlocal`** pointant sur `D:/CECPC/MASTORION/mastorion-v0`. `git fetch dlocal` / `git push dlocal <branche>` échangent des commits **sans réseau ni authentification** (pousser une branche non *checked out* sur D: est sans risque). Utile en dépannage ; le chemin normal reste `origin` (GitHub), désormais accessible — voir ci-dessous.
  - 🔑 **ACCÈS GITHUB — résolu le 2026-09-09, procédure à rejouer si le cache expire.** `origin` = `https://github.com/XTalandier/mastorion-v0.git` (dépôt de Xavier ; **l'utilisateur y a les droits d'écriture** — push de `feat/eho` réussi). ⚠ **Diagnostic à ne pas refaire** : un `git fetch origin` qui « part en timeout » n'est **PAS** une panne réseau (github.com répond en 0,3 s) — c'est **Git Credential Manager qui attend une saisie**. Mon shell impose `GIT_TERMINAL_PROMPT=0` et `GCM_INTERACTIVE=never` (garde-fous du harnais) et n'a **ni TTY ni GUI** : GCM tente le *device code*, échoue sur `/dev/tty`, et sort en 128. **Solution : ouvrir une vraie fenêtre PowerShell sur le bureau de l'utilisateur** (`Start-Process powershell -NoExit -EncodedCommand …`) en y **neutralisant les deux variables** (`$env:GIT_TERMINAL_PROMPT="1"` + `Remove-Item Env:GCM_INTERACTIVE`) puis en lançant `git ls-remote origin HEAD` → l'utilisateur valide dans son navigateur → identifiants stockés dans le gestionnaire Windows → **toutes mes commandes git passent ensuite sans sollicitation** (même avec `GIT_TERMINAL_PROMPT=0`, puisqu'aucun prompt n'est plus nécessaire). ⚠ `Start-Process` **hérite de l'environnement du parent** : sans la neutralisation explicite, la fenêtre affiche « user interactivity has been disabled » et l'opération échoue. Note : l'entrée `cmdkey` de GitHub Desktop (`GitHub - https://api.github.com/ViktorGlarak`) ne sert **pas** aux opérations git en ligne de commande, qui exigent l'entrée `git:https://github.com`.
...
## ⭐ App EHO (`apps/eho`, port 4204) — état durable au 2026-09-09 (mission EHO v2, 5 phases livrées)

> Détail chronologique et vérifications : `JOURNAL.md` §§ 2026-09-09/10. Branche `feat/eho`, 7 commits `b2e91ec…2628abd`.

- **Rôles** (colonne `role` libre, 0 migration) : `EHO_ANIM` (+ `APP_ADMIN`) = animateur ; `EHO_PLAYER` = joueur. Atterrissage par rôle (anim → `/modeles`, joueur → `/mon-eho`), gardes croisés, **toute autorisation re-vérifiée côté API** (jamais par masquage d'UI).
- **Critère structurel humain/persona** : un persona = compte `users` **sans aucun rôle** — c'est ce qui rend les humains intouchables par l'application d'un modèle, l'import et la suppression.
- **4 tables additives** (`eho.prisma` séparé, `db push`, base/social.prisma INTOUCHÉS) : `eho_state` (journal des modèles appliqués) · `eho_teams` / `eho_team_members` (cellules ; **EHO par cellule**, équipe d'1 possible ; joueur ∈ 1 cellule) · `eho_assessments` (fiche + placement par (cellule, persona) — références `users` par entier **sans relation Prisma**, sinon retouche de base.prisma).
- **Écrans** : `/modeles` (1ʳᵉ page anim — SKOLKAN PERSONA / VIERGE, application verrouillée : rôle + code retapé + **sauvegarde JSON auto avant suppression** + uploads jamais touchés) · `/trombinoscope` (officiel) · `/gestion` (CRUD personas, cellules+joueurs avec mdp affichés, **import/export Excel**) · `/comparaison` (officiel vs cellule, verdicts ✓/✗, « personas mal lus ») · `/mon-eho` (joueur : personas **dépouillés** — nom/handle/portrait seuls —, drag & drop CDK vers les pays, fiches de cellule ; équivalent clavier via la fiche).
- **Modèles d'EHO = données, hors git** : `apps/api/data/eho/templates/<code>/` (manifest + payload format import) ; `POST /api/eho/templates` **capture la base courante** → créer un modèle = opération de contenu. Sauvegardes d'application : `data/eho/backups/` (réimportables).

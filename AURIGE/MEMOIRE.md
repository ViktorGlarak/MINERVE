...
8. **⚠ `dayorder` ≠ vrai D+ : suivre la convention des onglets de l'instance.** Le `dayorder` d'un tweet/article baké pilote l'onglet jour du feed index_master. Cette numérotation est **propre à l'instance** et peut diverger du vrai D+ (ex. collab 7BB : D+39=30 Jun, D+40=1 Jul, D+41=2 Jul, pas de 29 Jun, max 41 — alors que le vrai D+33=23 Jun). **Avant de baker**, lire les `<button class="day-btn" data-dayorder="N">DATE · D+N</button>` de l'index_master cible et **mapper par DATE** (pas par calcul D+), sinon la card tombe hors onglet. Aligner aussi le label `time` sur cette convention.
9. **Export joueurs = kit vierge + livraison incrémentale (ne pas le bourrer).** Le kit JOUEURS (`GENERER_KIT_JOUEURS.py`) est généré épuré : `articles_data.js` seedé aux seuls STARTEX, puis `actus_index.js` charge des `lot_*.js` que la console MASTER produit **à chaque diffusion**. On NE recopie PAS les fichiers de données du collab dans le kit (au risque d'effacer les lots en cours). Pour livrer un inject aux joueurs : le **baker proprement dans le master (collab)** + s'assurer que les `.html` external existent (packaging), puis **publier/diffuser** au rythme de la synchromatrice. Les **traitants**, eux, utilisent le **collab** directement (dossier `mastaurige` copié) → baker dans le collab les sert. *(Personas → toujours réutiliser `avatars.js` (CONTROLE G camp tweet=avatar) ; artefacts HTML → cloner les chartes existantes (TM/courrier/tract/OTAN…), auto-contenus + `back-btn.js`.)*
10. **Splitter une card MELMIL sur plusieurs jours (même numéro d'inject).** Pour qu'un même code apparaisse sur **2 cards à 2 jours** (ex. une déception en 2 temps), utiliser un **sous-inject** : un enfant dont `getParentInjectCode` remonte au parent crée une 2ᵉ card pleine au même numéro, à son propre jour (CAS 1.5 de `melmil.js`). Ancien format : `XX.YY.ZZ[Lettre]i` (parent `XX.YY.ZZi`). **Format JEMM (depuis 2026-06-30) : `XX.YY.Izz<Lettre>`** (ex. `07.02.I08B` → parent `07.02.I08`) ; le parent doit avoir une card socle dans `melmil_data.js`. La card-titre affiche le numéro parent, le sous-item interne le suffixe. ⚠ Si tu renommes un inject sur un code **déjà occupé**, tu fusionnes les deux (doublon) — passe par le sous-inject. *(Capacité moteur portée 2026-06-30 ; détail `MASTAURIGE/MEMOIRE.md` ; à porter vierge.)*
11. **⭐ LEÇONS RETEX MINOTAURE 26 (2026-07-22) — transférables à tout AURIGE.** *(Fiche complète : `MINAUTORE\RETEX_MINOTAURE_26.md`, consolidée depuis le RETEX à chaud opérateur + le Hot Wash Up validé par les traitants + le RETEX KLE.)*
    - **a) Calibrer TACTIQUE — constat le plus unanime.** Sur un exercice de **niveau brigade**, un inject stratégique **ne produit aucune réaction** (« discours politico-stratégique : pas d'impact sur une brigade »). Le taux de scénarios stratégiques était **trop élevé** vs l'objectif. D'autant plus critique que le **nombre d'injects est volontairement limité** pour ne pas déborder les joueurs → chaque inject doit porter. *(Renforce la doctrine de calibration brigade ci-dessous.)*
    - **b) Fermer la BOUCLE DE RETOUR — la mesure la plus rentable.** Ne pas savoir **comment les joueurs ont traité les injects** = « **travailler dans le vent** » : pas de dynamique d'interaction, impossible d'approfondir, **démotivation des traitants**. → **Désigner nommément un responsable du retour sur traitement des injects** ; réclamer des **éléments chiffrés dans les assessrep**. Corollaire : l'impact ILI étant plus dur à mesurer que la LOG ou l'artillerie (« **so what ?** »), il faut **préparer la démonstration d'impact**, sinon l'ILI reste marginalisé face à une animation rivée sur le cinétique.
    - **c) Les GT doivent PRODUIRE, pas débattre.** Écueil constaté : 3 GT + 5 jours de GT3 surtout consacrés aux **objectifs**, injects décidés **dans les 2 derniers jours** → les opérateurs ne comprennent pas l'intérêt d'arriver si tôt. → **Sortir des GT avec des scénarios/storyboards déclinés sur une timeline précise**, et **ne pas geler la création après le GT** (la marge de manœuvre en cours d'exercice est un acquis à préserver).
    - **d) Casser les SILOS.** Sans partage **RENS / FORAD / DIV / ILI**, on produit **en double** (cas vécu : **les fiches bio faites deux fois**) et on transmet trop tard. → Mise en commun explicite + **organigramme des cellules hors SITCEN** (LOCON, DIV, joueurs) pour comprendre le **circuit de diffusion des produits** + un **référent dialogue** désigné.
    - **e) Ce qui a MARCHÉ, à reconduire tel quel** : **1 traitant par LIGNE D'OPÉRATION** (division du travail jugée pertinente) · **MASTAURIGE** comme colonne vertébrale (visualisation des injects par jour, création, conduite) · **matérialisation de l'EHO dans MASTAURIGE** (fiches bio cliquables = outil de coordination anti-doublon) · un **EHO profond est un ACTIF RÉUTILISABLE** d'un exercice à l'autre.
    - **f) Consignes stables.** Consignes peu nombreuses, **changeantes ou contradictoires**, flou sur les ciblages stratégique/opératif/tactique et **excès de correctifs** ont coûté plus cher que les pannes techniques. → **Missions claires en début d'exercice, tâches précises en début de journée.**
    - **g) Prérequis MATÉRIELS à exiger au montage** (sinon la production est bridée) : **accès internet ouvert au poste** (ou 2 postes : Sitaware/JEMM/PCA + internet), **postes de la cellule dans le même espace**, **carte de la zone d'exercice affichée**, traitants **en copie des mails** chef/adjoint, **formation aux logiciels** (+ guide type MASTAURIGE), **abonnements outils de production financés par le CECPC**, et un **ratio expert image / opérateurs équilibré** (1 pour 7 = goulot d'étranglement).
    - **h) Contenu — pistes d'enrichissement** : impliquer la **société civile** (syndicats, ONG anti-corruption type *White Gloves*, patronat, chefs d'entreprise, groupes religieux, milices, groupes terroristes) ; éviter les **redondances humanitaires** ; **synthétiser les country books** (~150 p. non lues par les joueurs) ; tenir un **annuaire des avatars** et **créer réellement les identités fictives sur les RS**.
    - **i) ⚠ JEMM** : jugé dispersant, non modifiable, à faible apport vs Mastorion/MASTAURIGE et **inadapté au niveau brigade** (a fortiori DIV/CA) ; **droits traitants à revoir** (bloqués en l'absence du chef et de l'adjoint). → Prévoir MASTAURIGE comme outil de travail réel et JEMM comme interface contrainte.
12. **🗂️ Hygiène de mémoire — MEMOIRE.md (durable) + JOURNAL.md (historique).** Une mémoire d'agent qui grossit sans fin devient **illisible avant de travailler** (cas vécu : `MASTAURIGE\MEMOIRE.md` monté à **397 Ko**, au-delà de la limite de lecture — alors que c'est le fichier à lire AVANT toute intervention). **Convention (depuis 2026-07-22)** : quand une mémoire d'agent **actif** dépasse ~**150 Ko** ou n'est plus lisible d'un coup, la **scinder** : `MEMOIRE.md` = **état durable** (règles, conventions, capacités, casting, identité — ce qu'on lit avant d'agir) ; `JOURNAL.md` = **compte-rendu chronologique daté** append-only (« ce que j'ai fait le … »), consulté au `grep`. **Écriture** : nouvelle règle/capacité durable → `MEMOIRE.md` ; CR daté → `JOURNAL.md`. Pour un exercice **CLOS**, poser une **bannière 🗄️ ARCHIVE** en tête de sa mémoire (elle devient une référence consultable, pas un fichier vivant) — cf. MINAUTORE (7BB) et GUILLAUME (2BB). *(Découpe faite via script Python à indices de ligne + assertions de garde, backup préalable — jamais à la main sur 400 Ko.)*
7. **⚠ Piège « Modifier » en mode COLLAB — baker avant de copier le dossier.** Sur la variante serveur collaboratif de MASTAURIGE, éditer un tweet/article **du socle** via le bouton « ✎ Modifier » change la **card** mais **n'écrit pas dans le fichier source** (`tweets_data.js`/`articles_data.js`) : la modif est un **overlay localStorage** (`mastaurige-edits`) persisté dans `etat_mastaurige.json` à la **racine du serveur, hors du dossier `mastaurige/`** → elle **ne voyage pas** quand on recopie/redistribue `mastaurige/` (le contenu retombe à l'ancienne version). **Réflexe** : avant toute copie/redistribution, **replier les overlays dans le fichier source** (outil `OUTILS\BAKER_EDITIONS.py`, `--dry-run` pour aperçu) afin d'avoir **une seule vérité durable**. Vaut pour les overrides de date au drag calendrier (`card-day-`/`card-time-`). *(Pour les contenus pré-rédigés riches, préférer dès le départ l'édition directe du `.html`/du fichier de données plutôt que « Modifier ».)*

...
> **[2026-06-23] 2 courriers HN individuels créés (injects 08.01.I02/I03)** d'après `PROMPT - 08.01.docx` : **Nadia PROMESY** (Maire HSarrebourg, 🔵) → 7e BB, demande d'évacuation préventive des ~10 000 hab. avant combats urbains ; **Irina VOLOSHYN** (Maire HToul, 🔵) → BFA, aide aux ~6 000 déplacés de HNancy + régulation conjointe des flux. Voix conformes aux bios (élues pro-ARN authentiques, registre administratif sobre). **Camp `bleu`** attribué à ces 2 courriers (≠ 08.01.I01 = `neutre`). Détail : `MINAUTORE\MEMOIRE.md` + `MASTAURIGE\MEMOIRE.md` [2026-06-23].
> **[2026-06-23] Nouveau persona @Stepan_Roubek (inject 08.02.I02)** — **Stépan Roubek**, **père de famille mercurianophone de la région de HSarrebourg**, penchant **🔴 pro-MER** (camp = registre MASTAURIGE/avatars.js, autorité). Profil : témoin local crédible, registre « j'ai vu de mes yeux / nos fils », **pas de slogan** (victimisation locale conforme à la doctrine). Emploi ILI : requalifie une reddition réelle (~50 MER rendus à la 7BB) en « capture forcée » et ouvre la question des droits des prisonniers (LO3/LO4). Distinct de @KolesnikovAndrei (commentateur international EN). Initiales SR. ⚠ ville **HSarrebourg** dans le contenu vs card socle JEMM 08.02.I02 = **HLUNEVILLE** (à réconcilier).
> **[2026-06-23] 08.01.I01 ré-aligné (PROMPT i01)** : courrier **Jean-Louis ADRIANE** (Maire HLunéville, **⚪ neutre**) → 7e BB, demande de **protection des écoles, hôpital, lieux de culte** (biens protégés DIH). Voix conforme à la bio (édile pragmatique apolitique, ni pro-ARN ni pro-MER). **Camp `neutre` confirmé** (Adriane ⚪) — la ligne HN a donc des camps mixtes assumés : I01 ⚪ (Adriane), I02 🔵 (Promesy), I03 🔵 (Voloshyn). Ancien scénario « corridors humanitaires » abandonné (ne correspondait pas au JEMM). Détail : `MINAUTORE\MEMOIRE.md` + `MASTAURIGE\MEMOIRE.md`.

### [2026-06-24] ⭐ ROSTER « REAR EHO » — maires de la zone arrière de HToul (intégrés trombino + vierge)
Source : `CREATION\06 - Arnland\HN FICHE BIO\20260311_DR_MINOTAURE_REAR EHO.docx`. **5 fiches** dans le doc dont **Irina Voloshyn (HToul) déjà au roster** (« VOIR BIO SITCEN » → non dupliquée) → **4 NOUVEAUX maires** de la **zone arrière** (sud-ouest de HToul ; carte du doc = secteur Toul/Vaucouleurs/Colombey). Camps décidés ici (ANALYSTE_ARN) d'après la rubrique « Relations avec l'ennemi » de chaque fiche :

| Figure | Rôle | Âge | Camp | Parti | Posture / levier |
|---|---|---|---|---|---|
| **Katia Volkov** | Maire **HVaucouleurs** | 42 | 🔵 pro-ARN | sans étiquette | **Relais actif des messages d'ARN** ; réseau d'entraide (proches arrêtés à l'Est) ; amie d'Irina Voloshyn ; active SOCMED (X perso + FB mairie) |
| **Marko Novak** | Maire **HFoug** | 67 | 🔵 pro-ARN (passif) | ASP / AWU | **« Soumission affichée »** : réquisition (gymnase, véhicules) subie, ralentit les procédures ; 22 ans de mandat ; veuf ; peu actif RS — *vulnérabilité ILI : image de collaboration* |
| **Denis Mirov** | Maire **HBlénod-lès-Toul** | 38 | ⚪ neutre (ambigu) | sans étiquette | Prof. histoire-géo ; **intérêt pour la culture de Mercure → comportement équivoque** = cible potentielle d'influence MER ; actif RS (culture) |
| **Sofia Renod** | Maire **HColombey-les-Belles** | 73 | 🔵 pro-ARN | ASP | **Refuse toute compromission avec les milices HFM** ; négociatrice redoutable ; influence sur Mirov et Novak ; active RS |

⚠ **Géo (décision cohérence)** : le texte écrit « Blénod-lès-Tours » mais la **carte du même document** et la zone (HToul) → **HBlénod-lès-Toul** retenu. Villes H-préfixées comme le reste du roster (HToul, HSarre-Union…).
✅ **Intégration trombino** : `parse_rear_eho()` (liste **curée** dans `generer_trombino_bios.py`, sans dépendance fichier) → **bios.js = 55 fiches** (Arnland 31). **Photos** découpées de la grille composite du `.docx` (recoupées par âge : Volkov=« Marchand 42 », Novak=« Vautrin 67 »/« Marcel », Mirov=« Leroux 38 », Renod=« Renaudin 73 ») → `Sites\Trombinoscope\img\arn_{katia_volkov,marko_novak,denis_mirov,sofia_renod}.jpg` + **rendu photo ajouté** à la bande et au pop-up (champ `photo`, repli initiales). Régénéré sur **MINOTAURE 7BB** (LOCALSTORAGE) **+ vierge** `D:\CECPC\MASTAURIGE\LOCALSTORAGE_WEB_VERSION` (générateur synchronisé au passage). Détail technique : `MASTAURIGE\MEMOIRE.md` [2026-06-24].

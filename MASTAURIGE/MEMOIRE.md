...
**⭐ STARTER PACKAGE — code réservé `00.00.00i` (2026-06-18, demande utilisateur) :** convention = un lot d'articles « ligne de conduite politique » donné aux joueurs au démarrage (7BB : le 23 juin / D+33), **sans vrai numéro** → code **réservé `00.00.00i`** ⇒ **carte incident GHOST** dans MELMIL (série `00` inconnue → ligne **r7 GHOST**, automatique), cartes colorées **par camp** (autorité qui parle).
- **Convention dans le moteur (les 3 instances : vierge + collab + 7BB)** : `MELMIL\melmil.js` — la carte-incident dont le code commence par `00.00.00` est **étiquetée « 📦 STARTER PACKAGE — ligne de conduite politique »** (au lieu de `inj.sujet`). `moteur\createur.js` — `CR_previewArticle` ouvre désormais le **vrai `.html` en iframe** si la création a `external:true` + `file` (sinon rendu data habituel) → un article riche pré-rédigé s'affiche tel quel (animateur ET, via `joueur.js`, joueurs).
- **Articles 7BB (instance 7BB + copie collab uniquement, PAS la vierge)** : `moteur\starter_package.js` (chargé dans `index_master.html` après `articles.js`) = **5 créations auto-injectées** (idempotent par id, respecte la corbeille) taguées `num:"00.00.00i"`, `external:true`, `file` = les 5 `.html` déjà adaptés (TM/Olamao 🔴 · TV4/Pallesson 🔵 · HEX/Rutte 🔵 · ONU/Guterres ⚪ · TV4/Guterres ⚪), `dayorder:"33"` (23 juin).
- ✅ Tests : `node --check` 8/8 · harness loader = 5 créations, idempotent, num/camp/external OK. ⏳ **Valider en NAVIGATEUR** : cartes dans le fil + ouverture du `.html` + carte ghost `00.00.00i` « STARTER PACKAGE » dans MELMIL. ⚠ **Régénérer le kit JOUEURS** pour qu'il embarque `starter_package.js` + les `.html`. Sauvegardes : `D:\CECPC\_cbtmp\starter_bak`.

...
> 📁 **Mémoire MASTAURIGE en 2 fichiers (depuis 2026-07-22) :**
> - **`MEMOIRE.md`** (ce fichier) = **ÉTAT DURABLE** : règles, conventions, capacités de l'outil, architecture, registres. **À LIRE avant toute intervention MASTAURIGE.** (~83 Ko, lisible d'un coup.)
> - **`JOURNAL.md`** = **HISTORIQUE chronologique** append-only (évolutions datées, récupérations, portages, sessions de bake). À **consulter au besoin** (grep), jamais à lire en entier. **Fait foi pour l'historique.**
>
> **Où écrire ?** Une nouvelle **règle / convention / capacité durable** → ici. Un **compte-rendu daté** de ce qui a été fait → **`JOURNAL.md`**.
...
*(🗂️ Le **journal d'activité daté** — récupérations d'injects, portages, créations d'outils, sessions de bake — a été déplacé dans **`MASTAURIGE\JOURNAL.md`** § « Journal d'activité » le 2026-07-22. Les **conventions et règles durables** restent ci-dessous.)*
...
## 🗂️ Historique des évolutions de l'outil → `JOURNAL.md`

Le changelog chronologique détaillé (ex-section « ⭐ v0.2 — état & évolutions — FAIT FOI », ~280 Ko) vit désormais dans **`MASTAURIGE\JOURNAL.md`**, qui **fait foi pour l'historique de l'outillage**.
Le présent `MEMOIRE.md` conserve l'**état durable** (règles, conventions, capacités, architecture, registres).
👉 Pour un détail d'implémentation daté (« comment a-t-on fait X ? », « pourquoi tel gotcha ? ») → **`grep` dans `JOURNAL.md`**.

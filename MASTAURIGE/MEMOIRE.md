...
### Avatar media social — BC1 Bella Russia (ajouté 2026-05-28)

| Handle | Nom affiché | Camp | Avatar | Rôle | Injects |
|---|---|---|---|---|---|
| @BC1_News | BC1 Bella Russia | 🔴 Rouge | Initiales "BC1" — background `#12125A` (navy BC1), texte blanc, `font-size: 0.6em`, `font-weight: 900` | Compte social officiel de BC1 — dépêches et témoignages en temps réel | 07.05.03i (Ci, Di) |
...
|---|---|---|---|---|---|
| @S_Tikhanov | Svetlana Tikhanov | 🔵 Bleu (opposition BR nuancée) | `avatars tweet/Svetlana Tikhanov - Nouvelle pahonie - photo avatar.png` | Réaction discours Youkachenko — fracture interne BR | P-27, **07.05.03Ai** |
...
| **Youkachenko** | 🔴 Rouge | Président BR | Soutien MER — discours anti-OTAN souveraniste |
...
| BC1 | BC1 | 🔴 | `#BC1` — source BR — Youkachenko parle via BC1 |
...
| `melmil_data.js` | Auto-généré par le PS1 — NE PAS MODIFIER manuellement |
| `generate_data.ps1` | Lit `MELMIL.xls` via Excel COM → génère `melmil_data.js` |
| `Actualiser_MELMIL.bat` | Lance le PS1 avec `-ExecutionPolicy Bypass` — double-clic suffit |
...
**Règle :** Tout nouveau bouton de navigation ajouté à l'en-tête de `index_master.html` doit : (1) appeler `saveMasterState()` en `onclick`, (2) être ajouté aux 3 listes de nettoyage des fonctions `generateSplit`, `generateComplet`, `generateIntegral`.
...
| 3 | 🟡 Bella Russia | Régime Youkachenko + appareil sécuritaire · Opposition (Tikhanov, Saniki) · CEMA BR · Médias BR + arcs narratifs |
...
- Tweet escalade BR / rupture coalition → **LO 5**
...
- **Invisible dans les exports stagiaire :** supprimé automatiquement par les 3 fonctions generate (split/complet/intégral)
...
**Actuellement en attente (27 mai 2026) :** `@NOVUSORDOMUNDI` — tweet pillages Héming — attendait l'activation du scénario ressortissants BR. Voir `rappel_novusordomundi_attente.md`.
